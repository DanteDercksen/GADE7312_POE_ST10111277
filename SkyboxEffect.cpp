...
SkyboxEffect::SkyboxEffect(ID3D11Device* device) : m_dirtyFlags(uint32_t(-1)), m_constantBuffer(device)
{
...
void SkyboxEffect::SetMatrices(DirectX::FXMMATRIX, DirectX::CXMMATRIX view, DirectX::CXMMATRIX projection)
{
    m_view = view;
    m_proj = projection;
    m_dirtyFlags |= DirtyWVPMatrix;
